import { APIService, calculations, AppState } from '../orderv3/forexProcess.js?ver=1.9';
import { DropdownCache, ProgressManager, TemplateCache, CONSTANTS, currencyNames, formatAmount, loginManager, ProgressGenerator, ButtonLoader } from '../orderv3/common.js?ver=1.5';
...
export const ForexUIManager = {
    ...
    cacheStaticElements() {
        ...
        // Prevent back navigation continuously
        (function preventBack() {
            history.pushState(null, null, document.URL);
            window.addEventListener("popstate", function () {
                alert("You will lose all the changes you made");
            });
        })();
    ...
    setupAddBottomSheetListeners(type) {
        ...
        addProductBtn.addEventListener('click', async () => {
            if (addProductBtn.disabled || AppState.isProcessing()) {
                console.log('Button is disabled or processing in progress');
                return;
            }

            try {

                ForexUIManager.manageSetProcessingState(CONSTANTS.PROCESSING_STATES.ADDING_PRODUCT, true);

                const data = await APIService.addProduct('add');
                await this.manageProcessingData(data);

                // Close bottom sheet and wait for animation to complete
                await new Promise(resolve => {
                    this.closeBottomSheet();
                    // Wait for bottom sheet animation (300ms) plus a small buffer
                ...
                ForexUIManager.manageSetProcessingState(CONSTANTS.PROCESSING_STATES.ADDING_PRODUCT, false);
...
`;
    ...
    async handleNextBtn() {
        ...
        if(!AppState.getState('token', 'mainState')){
            location.href='/'
        }
        try {
            if (AppState.nextBtnState.status === CONSTANTS.ORDER_STATES.GET_RATES) {
            ...
            if (AppState.nextBtnState.status === CONSTANTS.ORDER_STATES.DELIVERY_DETAILS) {
                ...
                let ddPin = document.querySelector('#ddPin').value;
                ...
                AppState.setState('ddPin', ddPin, 'deliveryState');
                ...
                if (AppState.deliveryState.doorDelivery) {
                    ...
                    if (!ddPin || ddPin.trim() === "" || !ddPin.match(/^\d{6}$/)) {
                        removeAlertBelowElement(document.querySelector('#ddPin'))
                        insertAlertBelowElement(document.querySelector('#ddPin'), 'Enter a valid pin')
                        return
                    } else {
                        removeAlertBelowElement(document.querySelector('#ddPin'))
                    }
            ...
            if (AppState.nextBtnState.status === CONSTANTS.ORDER_STATES.CONTACT_DETAILS) {
                ...
                if (!AppState.contactData.travelDate) {
                    insertAlertBelowElement(document.querySelector('#datePickerWrapper'), 'Select a valid date')
                    return
                } else {
                    removeAlertBelowElement(document.querySelector('#datePickerWrapper'))
                }
            ...
            if (AppState.nextBtnState.status === CONSTANTS.ORDER_STATES.REVIEW_PAYMENT) {
                ...
                if (data.status) {
                    ...
                    ForexUIManager.manageSetProcessingState(CONSTANTS.PROCESSING_STATES.INITIAL_LOAD, false);
                    AppState.setState('token', null, 'mainState');
    ...
    async initializeDeliveryDetailsComponents() {
        ...
        if (data) {
            ...
            const cityInput = document.getElementById('ddCity');
            if (districtName) {

                districtName.textContent = data.selected_district || '';
            }
            if (cityInput) {
                cityInput.value = data.selected_district || '';
            ...
            document.querySelector('#deliveryCartTextContainer').style.display = 'inline-flex'
            document.querySelector('#getRatesCartTextContainer').style.display = 'none'
            let cartContentText = document.querySelectorAll('.cartContentText')
            if (data.order_within_hours || data.order_within_mins) {
                cartContentText.forEach(item => {
                    item.textContent = `${data.delivery_on} if you Order within ${data.order_within_hours} hrs ${data.order_within_mins} mins.`
                })
            } else {
                cartContentText.forEach(item => {
                    item.textContent = data.delivery_on
                })
            }
            ...
            Dropdown.init('cityDropDown', {
                ...
                onSelect: (value) => {
                    ...
                    document.querySelector('#selectedBranchStorePickup').textContent = `Location : ${value}`
                }
            });
    ...
    async initializeContactDetailsComponents() {
        ...
        if (data) {
            ...
            console.log(data.travel_date, 'data.travel_date');

            // if(AppState.contactData.travelDate==null){
            //     AppState.contactData.travelDate = window.getFormatedDate(new Date());
            // }
    ...
    async initializeSummaryComponents() {
        ...
        console.log(summaryData, 'summaryMN')
        ...
        if (summaryData.delivery_opted === '0') {
            ...
            console.log('calledc')
            doorDeliveryElement.style.display = "inline-flex";
    ...
    handleBackBtn() {
        ...
        switch (currentStatus) {
            case CONSTANTS.ORDER_STATES.DELIVERY_DETAILS:
                ...
                document.querySelector('#deliveryCartTextContainer').style.display = 'none'
                document.querySelector('#getRatesCartTextContainer').style.display = 'inline-flex'
