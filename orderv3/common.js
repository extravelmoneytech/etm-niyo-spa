...
export const TemplateCache = {
    ...
    async get(templateName) {
        ...
        try {
            const response = await fetch(`/orderv3/components/${templateName}.html`, { cache: "no-store" });
...
export const loginManager = {
    ...
    getMobileData() {
        let countryCode = Dropdown.getValue('contryCodeMain').getAttribute('mob-code').replace(/^\+/, '')
    ...
    async handleSendOtp(otpMode) {
        ...
        if (countryCode === "" && mobNumber==="") {
            insertAlertBelowElement(this.otpInputContainer, 'Invalid mobile number');
            return;
        } else if (mobNumber === "" || !/^\d+$/.test(mobNumber) || mobNumber.length !== 10) {
            insertAlertBelowElement(this.otpInputContainer, 'Invalid mobile number');
            return;
        } else {
...
export const ProgressGenerator = {
    generateProgressContainer(progressStates) {
        ...
        templateContainer.parentNode.insertBefore(progressContainer, templateContainer);
    },
    ...
    generateMobileProgress() {
        ...
        progressBar.innerHTML = `
            <div class="w-10 aspect-square bg-white rounded-3xl border-2 border-primary flex-col justify-center items-center gap-2.5 inline-flex">
                <div>
            ...
            <div class="flex flex-1 shrink gap-2.5 self-stretch my-auto h-0.5 border border-primary basis-4 w-[198px]" role="progressbar"></div>
        `;
